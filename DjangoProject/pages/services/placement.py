...
from pages.services import scaner
...
Модуль: placement.py
Назначение:
    Реализация бизнес-логики операции размещения катушек на складе.

Описание работы:
    Функция run_placement() запускается по команде оператора (через веб-интерфейс Django).
    Выполнение происходит в отдельном потоке, чтобы не блокировать веб-сервер.

    В процессе работы функция:
    1. Фиксирует начало операции размещения в журнале операций (таблица IH_LOG).
    2. Последовательно выполняет шаги алгоритма размещения.
    3. На каждом шаге пишет диагностическое сообщение в журнал операций.
    4. После завершения операции фиксирует успешное окончание работы.

Логирование:
    Для записи сообщений используется общий модуль logInsert.
    Все сообщения сохраняются в базе данных PostgreSQL и отображаются
    в интерфейсе оператора в режиме реального времени (через периодический опрос).
...
Примечания:
    - Модуль не содержит логики работы с HTTP или Django.
    - Может использоваться как из веб-приложения, так и из standalone-скриптов.
"""
operator = "ivanov"
workstation_id = "WS-01"

def run_placement() -> dict:
    logInsert.ih_log("СТАРТ Операции размещения", operation="PLACEMENT", source="placement", user="ivanov")
    operator = "ivanov"
    workstation_id = "WS-01"

    # 0. Инициализируем IH_led_task из IH_bin.mode_id
    logInsert.ih_log("Запуск размещения функция init_led_task_from_bin_mode", operation=operator, source="Размещение", user="ivanov")
    init_result = init_led_task_from_bin_mode()
    logInsert.ih_log(f"{init_result}", operation=operator, source="Размещение", user="ivanov")
    print("[INIT_LED_TASK]", init_result)

    # 1. Открываем операцию размещения PLACEMENT
    logInsert.ih_log("Запуск размещения функция open_placement_operation", operation=operator, source="Размещение", user="ivanov")
    placement_res = open_placement_operation(
        operator=operator,
        workstation_id=workstation_id
    )
    print("[OPEN_PLACEMENT]", placement_res)
    logInsert.ih_log(f"{placement_res}", operation=operator, source="Размещение", user="ivanov")

    if not placement_res.get("ok"):
        print("[FATAL] Не удалось открыть операцию размещения. Выходим.")
        logInsert.ih_log(f"Не удалось открыть операцию размещения. Выходим.", operation=operator, source="Размещение", user="ivanov")
    ...
    logInsert.ih_log(f"Операция размещения открыта , id={op_id}", operation=operator, source="Размещение", user="ivanov")

    # 2. Сканирование катушки
    logInsert.ih_log(f"Запуск размещения функция placement_step_1() Сканирование катушки ", operation=operator, source="Размещение", user="ivanov")
    step1 = placement_step_1()
    print("[STEP1_SCAN]", step1)
    logInsert.ih_log(f"{step1}", operation=operator, source="Размещение", user="ivanov")
    ...
    if not step1.get("ok"):
        ...
        logInsert.ih_log(f"Сканирование не удалось, закрываем PLACEMENT как CANCELLED.", operation=operator, source="Размещение", user="ivanov")

        idle_res = open_idle_operation(operator=operator, workstation_id=workstation_id)
        print("[OPEN_IDLE]", idle_res)
        logInsert.ih_log(f"Переход в режим IDLE", operation=operator, source="Размещение", user="ivanov")
        raise SystemExit(0)

    # 3. Проверка катушки в базе
    logInsert.ih_log(f"Запуск размещения функция check_barcode_in_db() Проверка катушки в базе", operation=operator, source="Размещение", user="ivanov")
    step2 = check_barcode_in_db()
    print("[STEP2_CHECK_DB]", step2)
    logInsert.ih_log(f"{step2}", operation=operator, source="Размещение", user="ivanov")
    ...
    if not (step2.get("ok") and step2.get("exists")):
        ...
        print("[CLOSE_PLACEMENT]", close_res)
        logInsert.ih_log(f"Товар по штрихкоду не найден, закрываем PLACEMENT как NO_ITEM.", operation=operator, source="Размещение", user="ivanov")
    ...
    logInsert.ih_log(f"Запуск размещения функция get_available_bin_ids_for_barcode() Подсветка доступных ячеек белым", operation=operator, source="Размещение", user="ivanov")
    bins_result = get_available_bin_ids_for_barcode()
    print("[STEP3_GET_BINS]", bins_result)
    logInsert.ih_log(f"{bins_result}", operation=operator, source="Размещение", user="ivanov")
    ...
    if not (bins_result.get("ok") and bins_result.get("bin_ids")):
        ...
        print("[CLOSE_PLACEMENT]", close_res)
        logInsert.ih_log(f"Нет доступных ячеек, закрываем PLACEMENT как NO_BINS.", operation=operator, source="Размещение", user="ivanov")
    ...
    logInsert.ih_log(f"Запуск размещения функция placement_step_wait_sensor_and_place Ожидание Sensor и размещение катушки", operation=operator, source="Размещение", user="ivanov")
    final_result = placement_step_wait_sensor_and_place(bins_result, step2)
    print("[STEP4_PLACE_BY_SENSOR]", final_result)
    logInsert.ih_log(f"{final_result}", operation=operator, source="Размещение", user="ivanov")
    ...
    if final_result.get("ok") and final_result.get("updated_bin"):
        ...
        print("[CLOSE_PLACEMENT]", close_res)
        logInsert.ih_log(f"Размещение подтверждено, закрываем PLACEMENT как DONE.", operation=operator, source="Размещение", user="ivanov")
        ...
        print("[CLOSE_PLACEMENT]", close_res)
        logInsert.ih_log(f"Катушка не была размещена, закрываем PLACEMENT как NOT_PLACED.", operation=operator, source="Размещение", user="ivanov")
        logInsert.ih_log(f"{close_res}", operation=operator, source="Размещение", user="ivanov")
                         
    
    logInsert.ih_log("ФИНИШ Операции размещения", operation="PLACEMENT", source="placement", user="ivanov")
    return {"ok": True}
