"""
Принципы, чтобы не словить проблемы

Один объект DB на процесс (и передавать его туда, где нужен)

Для SQLite: включить WAL и таймаут (иначе “database is locked”)

Не вызывать БД из state.py (state не должен знать про БД)

Логирование: лучше событиями (insert), а не “апдейтить каждую секунду”
"""
